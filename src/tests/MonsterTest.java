...
class MonsterTest {
	...
	/**
	 * Initializes the test Monster and Hero with a mock random instance before each
	 * test.
	 */
	@BeforeEach
	void setUp() {
	...
	void testCastAttackOnReducesHealth() {
		...
		when(myMockRandom.nextDouble()).thenReturn(0.5);
		when(myMockRandom.nextInt(10, 21)).thenReturn(15);

		myTestMonster.castAttackOn(myTestHero);
		myTestHero.setCurHealthPoints(myTestHero.getCurHealthPoints() - 15);
	...
	void testMonsterDoesNotAttackWhenDead() {
		myTestMonster.takeDamage(myTestMonster.getCurHealthPoints());
		myTestHero.setCurHealthPoints(100);
	...
	void testPropertyChangeListenerFiresOnHeal() {
		...
		verify(mockListener).propertyChange(argThat(event -> event.getPropertyName().equals("health")
				&& event.getOldValue().equals(50) && event.getNewValue().equals(56)));
	}
}
